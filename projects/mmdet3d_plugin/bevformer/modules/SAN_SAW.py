...
class SAN(nn.Module):
    ...
    def set_class_mask_matrix(self, normalized_map):
        ...
        #     mask_matrix = torch.flatten(torch.zeros(b, c, h, w).to(normalized_map))
        #     mask_matrix[indices] = 1
        # except:
        #     mask_matrix = torch.ones(var_flatten.size()[0]).to(normalized_map)
    ...
    def forward(self, x, masks):
        ...
        for idx, i in enumerate(self.selected_classes):
            mask = torch.unsqueeze(masks[:,i,:,:],1)    # B,1,H,W
            mid = x * mask  # B,C,H,W
            ...
            out = mid*atten # B,C,H,W
            heatmap = torch.mean(out, dim=1, keepdim=True)  # B,1,H,W

            class_region = self.set_class_mask_matrix(heatmap)  # B,1,H,W
...
class SAW(nn.Module):
    def __init__(self, dim, selected_classes, relax_denom=0, classifier=None):
    ...
    def get_mask_matrix(self, x):
        i = torch.eye(self.C, self.C).to(x)
        reversal_i = torch.ones(self.C, self.C).triu(diagonal=1).to(x)
        num_off_diagonal = torch.sum(reversal_i)
        if self.relax_denom == 0:
            margin = 0
        else:
            margin = num_off_diagonal // self.relax_denom
        return i, reversal_i, margin, num_off_diagonal
    ...
    def forward(self, x):
        if self.training:
            ...
            eye, mask_matrix, margin, num_remove_cov = self.get_mask_matrix(x)
            SAW_loss = torch.FloatTensor([0]).to(x)
