...
class NuScenesTraj(object):
    def __init__(self,
        ...
        self.predict_steps = predict_steps  # 12
        self.planning_steps = planning_steps# 6
        self.past_steps = past_steps        # 4
        self.fut_steps = fut_steps          # 4
        self.with_velocity = with_velocity
        self.CLASSES = CLASSES
        self.box_mode_3d = box_mode_3d
        self.predict_helper = PredictHelper(self.nusc)
        self.use_nonlinear_optimizer = use_nonlinear_optimizer  # True
    ...
    def get_l2g_transform(self, sample):
        ...
        yaw_angle = quaternion_yaw(ego_rotation) / np.pi * 180
        if yaw_angle < 0:
            yaw_angle += 360

        return l2e_r_mat, l2e_t, e2g_r_mat, e2g_t, yaw_angle

    def get_sdc_planning_label(self, sample_token):
        sd_rec = self.nusc.get('sample', sample_token)
        l2e_r_mat_init, l2e_t_init, e2g_r_mat_init, e2g_t_init, ego_yaw_init = self.get_l2g_transform(sd_rec)
        

        planning = []   # plan_steps*[box_3d]  未来帧sdc_box 在ref帧lidar坐标系下
        ego_yaw = []
        for _ in range(self.planning_steps):
            ...
            if next_annotation_token=='':
                break
            sd_rec = self.nusc.get('sample', next_annotation_token)
            l2e_r_mat_curr, l2e_t_curr, e2g_r_mat_curr, e2g_t_curr, ego_yaw_curr = self.get_l2g_transform(sd_rec)  # (lidar to global at current frame)
            ...
            # to bbox under initial lidar frame, first inverse translate, then inverse rotate
            ...
            # ego yaw
            yaw = ego_yaw_curr - ego_yaw_init
            ego_yaw.append(yaw)

        planning_all = np.zeros((1, self.planning_steps, 3))        # 1,L,3  x,y,yaw
        planning_mask_all = np.zeros((1, self.planning_steps, 2))   # 1,L,2  valid_frame=1
        ...
        if mask.sum() == 0:
            ...
            command = 2 #'FORWARD'
        command = np.repeat(command, self.planning_steps)

        return planning_all[0], planning_mask_all[0], command
        # Lout=1,3   x,y,yaw    ref帧lidar坐标系下
        # Lout=1,2   valid_frmae=1