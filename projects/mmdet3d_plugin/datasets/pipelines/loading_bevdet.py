...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def __init__(self, data_config, is_train=False, using_ego=True, colorjitter=False,
        ...
        self.trans_only = trans_only    # False
        self.load_depth = load_depth    # True
    ...
    def get_seq_inputs(self, results, flip=None, scale=None):
        ...
        sequence_length = results['time_receptive_field']
        ...
        for counter in range(sequence_length):
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                imgs.append(self.normalize_img(img, img_norm_cfg=self.img_norm_cfg))    # img_h,img_w,3
                # intrins.append(intrin)  # 4*4
                # rots.append(rot)        # 3*3 lidar2cam_rotation
                # trans.append(tran)      # 3 lidar2cam_translation
                # post_rots.append(post_rot)  # 3*3
                # post_trans.append(post_tran)    # 3
                # sensor2sensors.append(sensor2lidar) # 4*4 lidar2cam

            results['img_metas'][counter]['img_shape'] = [img.shape[1:] for img in imgs]
        ...
        imgs_seq = torch.stack(imgs_seq)    # seq_len, Ncams, img_h,img_w,3
        # rots_seq = torch.stack(rots_seq)    # seq_len, Ncams, 3,3   lidar2cam_rotation
        # trans_seq = torch.stack(trans_seq)  # seq_len, Ncams, 3     lidar2cam_translation
        # intrins_seq = torch.stack(intrins_seq)  # seq_len, Ncams, 4,4   camera_intrinsic
        # post_rots_seq = torch.stack(post_rots_seq)  # seq_len, Ncams, 3,3   lidar2cam_rotation变换后的图片
        # post_trans_seq = torch.stack(post_trans_seq)# seq_len, Ncams, 3     lidar2cam_translation变换后的图片
        # gt_depths_seq = torch.stack(gt_depths_seq)  # seq_len, Ncams
        # sensor2sensors_seq = torch.stack(sensor2sensors_seq)  # seq_len, Ncams, 4,4   lidar2cam
